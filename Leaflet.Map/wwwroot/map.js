let showOrUpdate = (elementId, zoom, markers) => {
    ...
    if (!elem.map) {
        ...
        L.tileLayer('https://api.mapbox.com/styles/v1/masumbd/tiles/0/0/0?access_token=sk.eyJ1IjoibWFzdW1iZCIsImEiOiJjbGZ3eHB1dnUwN2RyM2VwaDU4eDUzaTN2In0._YJBce7-p6y4ejVQz0xF2A', 
            {
                ...
                tileSize: 512,
                zoomOffset: -1,
                accessToken: 'sk.eyJ1IjoibWFzdW1iZCIsImEiOiJjbGZ3eHB1dnUwN2RyM2VwaDU4eDUzaTN2In0._YJBce7-p6y4ejVQz0xF2A'
            }).addTo(elem.map);
    }
...
// Same number of markers, so update positions/text without changing view bounds
    markers.forEach((marker, index) => {
        animateMarkerMove(
            map.addedMarkers[index].setPopupContent(marker.description),marker,4000);
    });
}
};
...
let animateMarkerMoveFrame = (marker) => {
    ...
    if (proportionCompleted < 1) {
        marker.existingAnimation.callbackHandle = window.requestAnimationFrame(() => animateMarkerMoveFrame(marker));
    }
}
